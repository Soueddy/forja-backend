...
app.post('/webhook/cakto', async (req, res) => {
    try {
        ...
        if (event === 'payment.approved' || event === 'order.approved' || event === 'payment_approved' || event === 'order_approved' || event === 'subscription_approved' || event === 'subscription_active') {
            ...
            // 1. Criar o usuário no Firebase Authentication (Aba Users) para que ele possa redefinir a senha
            try {
                // Tenta criar o usuário com uma senha aleatória provisória
                ...
                console.log(`👤 Usuário criado no Authentication: ${customerEmail}`);
            } catch (authError) {
                // Se o usuário já existir, apenas logamos e continuamos o fluxo
                if (authError.code === 'auth/email-already-exists') {
                    console.log(`ℹ️ Usuário já existe no Authentication: ${customerEmail}`);
                } else {
                    console.error("❌ Erro ao criar usuário no Auth:", authError);
                }
            }

            // 2. Set ou update no Firebase Firestore (Banco de Dados)
            await userRef.set({
                name: customerName,
                email: customerEmail,
                status: 'active',
