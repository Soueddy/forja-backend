...
app.post('/webhook/cakto', async (req, res) => {
    try {
        ...
        if (payload.event === 'payment.approved' || payload.event === 'order.approved') {
        ...
        } else if (payload.event === 'subscription.canceled' || payload.event === 'payment.refunded' || payload.event === 'chargeback') {
